...
extension UIImage {
    ...
    //给全屏展示的图片缩放
    public func sfAndPanImage(imageView: UIImageView!) {
        //添加缩放手势
        let sftap = UIPinchGestureRecognizer.init(target: self, action: #selector(sfImage))
        imageView.addGestureRecognizer(sftap)
    ...
    //修改锚点
    public func adjustAnchorPoint(_ tap: UIPinchGestureRecognizer) {
        if tap.state == .began {
            let piece = tap.view!
            //获得当前手势在view上的位置
    ...
    @objc private func panImage(_ pan: UIPanGestureRecognizer) {
        ...
        if pan.state == .began || pan.state == .changed {
            ...
            if position.x < 0 && imageView.frame.maxX <= UIScreen.main.bounds.size.width {
                position.x = 0
            }
            ...
            if position.y < 0 && imageView.frame.maxY <= UIScreen.main.bounds.size.height {
                position.y = 0
            }
