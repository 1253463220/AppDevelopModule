...
class GenericPanGestureHandler: NSObject {
    ...
    @objc private func panAction(gesture: UIPanGestureRecognizer) {
        ...
        afterY = (self.handleView?.frame.origin.y ?? 0) + distance
        ...
        case .changed:// 识别器已经接收到触摸，并且识别为手势改变(Changed)
            ...
            self.handleView?.frame.origin.y = afterY;
    ...
    private func autoBackLocation(velocity: CGPoint) {
        ...
        if view.frame.origin.y < self.minY * 0.5 + self.midY * 0.5 {
            if velocity.y > 50 {
        ...
        }else if (view.frame.origin.y >= self.minY * 0.5 + self.midY * 0.5) && (view.frame.origin.y < self.maxY * 0.5 + self.midY * 0.5) {
            if velocity.y > 50 {
                self.directionUp = false
                self.levelType = .max
                ...
                self.directionUp = view.frame.origin.y > self.midY
                self.levelType = .mid
            }
        }else if view.frame.origin.y >= self.maxY * 0.5 + self.midY * 0.5 {
            if velocity.y < -50 {
                self.directionUp = true
                self.levelType = .mid
            }else {
    ...
    var levelType: BottomBarLevelType = .mid {
        didSet {
            UIView.animate(withDuration: 0.3, animations: {
                ...
                case .min:
                    offsetY = (self.handleView?.frame.origin.y ?? 0) - self.minY
                    self.handleView?.frame.origin.y = self.minY - self.extraHeight
                    if self.offsetChangeBlock != nil {
                        self.offsetChangeBlock!(self.minY, offsetY,self.directionUp);
                    }
                case .mid:
                    offsetY = (self.handleView?.frame.origin.y ?? 0) - self.midY
                    self.handleView?.frame.origin.y = self.midY - self.extraHeight
                    if self.offsetChangeBlock != nil {
                        self.offsetChangeBlock!(self.midY, offsetY,self.directionUp);
                    }
                case .max:
                    offsetY = (self.handleView?.frame.origin.y ?? 0) - self.maxY
                    self.handleView?.frame.origin.y = self.maxY - self.extraHeight
                    if self.offsetChangeBlock != nil {
